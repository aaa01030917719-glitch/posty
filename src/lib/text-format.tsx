...
function renderInlineContent(
  ...
  while (remaining) {
    ...
    if (match.type === 'bold') {
      nodes.push(<strong key={key}>{renderInlineContent(match.value, `${key}-bold`, mediaById)}</strong>)
    } else if (match.type === 'italic') {
      nodes.push(<em key={key}>{renderInlineContent(match.value, `${key}-italic`, mediaById)}</em>)
    } else if (match.type === 'strike') {
      nodes.push(<del key={key}>{renderInlineContent(match.value, `${key}-strike`, mediaById)}</del>)
      ...
      nodes.push(
        ...
        >
          {renderInlineContent(match.value, `${key}-large`, mediaById)}
        </span>
      )
      ...
      nodes.push(
        <span key={key} className={getColorInlineClass(match.type)}>
          {renderInlineContent(match.value, `${key}-${match.type}`, mediaById)}
        </span>
      )
      ...
      nodes.push(
        ...
        >
          {renderInlineContent(match.label, `${key}-link`, mediaById)}
        </a>
      )
      ...
      nodes.push(
        ...
        >
          {getSafeMediaAlt(match.alt, mediaById.get(match.id), MEDIA_PREVIEW_LABEL)}
        </span>
      )
    }
...
function isAutoGeneratedMediaAlt(alt: string, media: FormattedTextMediaItem | undefined) {
  const fileName = media?.fileName?.trim()
  return Boolean(fileName) && alt.trim() === fileName
}

function getSafeMediaAlt(
  alt: string,
  media: FormattedTextMediaItem | undefined,
  fallbackLabel: string
) {
  return isAutoGeneratedMediaAlt(alt, media) ? fallbackLabel : alt || fallbackLabel
}
...
function renderMediaBlock(
  ...
  const altText = getSafeMediaAlt(alt, media, label)
      ...
      {blocks.map((block, index) => {
        if (block.type === 'text') {
          return (
            ...
            >
              {renderInlineContent(block.value, `text-${index}`, mediaById)}
            </div>
          )
        }
        ...
        if (block.type === 'heading') {
          return (
            <h2
              ...
              className="text-[30px] font-semibold leading-[1.25] text-[var(--color-text-primary)]"
            >
              {renderInlineContent(block.value, `heading-${index}`, mediaById)}
            </h2>
          )
        }
            ...
            >
              {block.items.map((item, itemIndex) => (
                <li key={`list-${index}-${itemIndex}`}>
                  {renderInlineContent(item, `list-${index}-${itemIndex}`, mediaById)}
                </li>
              ))}
            </ListTag>
          )
              ...
              <thead className="bg-[var(--color-bg-subtle)] text-[var(--color-text-primary)]">
                <tr>
                  {block.header.map((cell, cellIndex) => (
                    ...
                    >
                      {renderInlineContent(cell, `head-${index}-${cellIndex}`, mediaById)}
                    </th>
                  ))}
                </tr>
              </thead>
                ...
                {block.rows.map((row, rowIndex) => (
                  <tr key={`row-${rowIndex}`}>
                    {row.map((cell, cellIndex) => (
                      ...
                      >
                        {renderInlineContent(cell, `cell-${index}-${rowIndex}-${cellIndex}`, mediaById)}
                      </td>
                    ))}
                  </tr>
                ))}
